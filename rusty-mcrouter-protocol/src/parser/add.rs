...
mod tests {
    ...
    #[test]
    fn parse_request_add_with_flags_and_exptime() {
        let mut buf = BytesMut::from(&b"add k 42 3600 1\r\nv\r\n"[..]);
        assert_eq!(parse_request(&mut buf).unwrap().unwrap(), add(b"k", 42, 3600, b"v"));
        assert!(buf.is_empty());
    }

    #[test]
    fn parse_request_add_rejects_invalid_key() {
        let mut buf = BytesMut::from(&b"add \x01bad 0 0 3\r\nbar\r\n"[..]);
        assert!(matches!(
    ...
    #[test]
    fn parse_request_add_rejects_noreply() {
        let mut buf = BytesMut::from(&b"add foo 0 0 3 noreply\r\nbar\r\n"[..]);
        assert!(matches!(
            parse_request(&mut buf),
