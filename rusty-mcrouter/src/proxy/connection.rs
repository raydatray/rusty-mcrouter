...
/// One client connection's lifecycle: parse pipelined requests, dispatch each to
/// a proxy (local inline or remote via the proxy queue), and write replies back
/// in request order. Mirrors `Proxy::run` on the connection-owning side.
pub struct Connection<S> {
    reader: ReadHalf<S>,
    writer: WriteHalf<S>,
    // routing context (set once)
    current_id: usize,
...
impl<S: AsyncRead + AsyncWrite + Unpin> Connection<S> {
    ...
    /// Choose a proxy for `req` and spawn the routing task. Same-thread requests
    /// route inline; remote requests cross into the target proxy's queue.
    fn submit(&self, seq: usize, req: Request) {
        ...
        tokio::task::spawn_local(async move {
            let reply = if same_thread {
                route_or_error(&route, req).await
            } else {
...
async fn route_or_error(route: &Rc<dyn DynRoute>, request: Request) -> Reply {
    route
        .route_dyn(request)
        .await
