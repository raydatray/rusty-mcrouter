...
pub fn proxy_thread_main(
    ...
    local.block_on(&rt, async move {
        ...
        // Bind the listening socket if this thread owns one. `listener` couples the
        // bound server with the socket queues it dispatches to, so worker-only
        // threads carry neither.
        let listener = match listener_config {
            Some(ListenerConfig {
                ...
                let server = match bind_result {
                    ...
                    Err(e) => {
                        let _ = ready_tx
                            .send(Err(anyhow::anyhow!("bind({listen_addr}) failed: {e}")));
        ...
        // Each thread builds its own route graph; the `Rc<dyn DynRoute>` is
        ...
        // Proxy actor: drains this thread's message queue (requests routed here by
        // peer threads). Same-thread requests bypass it inside the connection task.
        let proxy = Proxy {
            id: proxy_id,
        ...
        // Connection worker: drains handed-off sockets and serves each connection.
