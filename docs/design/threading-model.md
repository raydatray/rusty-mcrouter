...
That handle is the producer side. The consumer is a **`Proxy` actor** that owns
the receiver and the route graph and drains its mailbox — our
...
struct Proxy {
    ...
    rx: tokio::sync::mpsc::Receiver<ProxyMessage>,
    // grows here as features land: stats, config epoch, health/TKO — not before they exist
}

impl Proxy {
    async fn run(mut self) {                  // by value: the task owns its state for life
        while let Some(msg) = self.rx.recv().await {
            match msg {
                ProxyMessage::Request(req) => {
                    let route = Rc::clone(&self.route);  // clone before spawn; no self-borrow leaks
...
**Why a named struct instead of a free `run_proxy_queue(rx, route)` function:**
today it is a zero-cost rename — the loop already takes its state by value — but it
stops the signature churn that arrives as the proxy accretes state: `(rx, route)`
would otherwise grow into `(id, rx, route, stats, config_epoch, …)` and touch
every call site. It also makes the code mirror mcrouter's `Proxy` 1:1. Two
boundaries so this doesn't over-correct:

- **`serve_session` stays a free function.** It is per-connection, spawned per
  socket, and holds a `ProxyHandle` to submit through — it is *not* the proxy
  actor. Don't fold it into `Proxy`.
- **Don't pre-add empty fields.** Give the struct the fields it actually has
  (`id`, `route`, `rx`) and add `stats` / config-swap / health when those
  features land. `run(self)` makes growth free, so stubbing them now is pure noise.

This is the **same handle + actor pattern as the backend client**
([`../mcrouter/backend-client.md`](../mcrouter/backend-client.md)): `ProxyHandle`
is to `Proxy` what `Client` is to `ClientConnection` — a cheap, cloneable
`mpsc::Sender` handle in front of a state-owning `run(self)` loop, with the reply
riding back over a `oneshot`. The skeletons match; the loop internals
deliberately differ:

| | `ClientConnection` | `Proxy` |
|---|---|---|
| concurrency | one serial `select!` loop over one socket | drains mailbox, `spawn_local`s a task **per request** |
| reply matching | central FIFO `pending` (wire replies are positional) | none; each `reply_tx` rides inside its own message |
| `Send`-ness | all-`Send`, `tokio::spawn`-able on any thread | holds an `Rc` route graph, so `!Send` and `spawn_local`-pinned to its thread |
| cardinality | one `Client`, one connection, one backend | `N` `ProxyHandle`s in a `ProxySet`, `N` proxy threads |
| ordering | preserved in the loop (FIFO) | not here; restored in `serve_session`'s reorder buffer |

Note the **reply-drop safety** the `oneshot` buys: if a route task panics or is
cancelled, its `reply_tx` drops, `send_request`'s `reply_rx.await` errors, and the
caller gets a `ServerError` instead of a hung connection (a dropped `oneshot` is
our only signal — the proxy-side analogue of the client's `fail_all_pending`).
...
| `Proxy::messageReady` | `Proxy::run` recv loop |
...
1. **Add `ProxyMessage`, `ProxyRequest`, `ProxyHandle`, and the `Proxy` actor
   (`run(self)`).** Keep accepted-socket distribution as-is. First
   behavior-preserving step: route every request through the *current* proxy's
   queue.
