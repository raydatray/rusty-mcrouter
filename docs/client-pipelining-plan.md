...
|---|---|---|---|
| API shape | request context waits on baton | `Client::send(&mut self, ...)` | cloneable `Client` handle + socket-owning `ClientConnection` |
| Backend concurrency | many in-flight requests per socket | one request per socket RTT | one `ClientConnection` writer/reader + FIFO waiters |
...
bounded command queue
  │
  ▼
ClientConnection owns backend socket
...
Naming convention for the planned Rust code:

```text
Client:
  public cloneable producer handle used by routes/tasks

ClientConnection:
  private socket owner; owns TcpStream, read buffer, command receiver,
  pending reply FIFO, and connection error state
```
...
counter inside `ClientConnection` for `max_inflight`.
...
impl ClientConnection {
    async fn run(mut self) {
        loop {
            tokio::select! {
                Some(ClientCommand::Request { request, reply_tx }) = self.rx.recv() => {
                    let mut out = BytesMut::new();
                    request.serialize_into(&mut out);

                    if let Err(err) = self.stream.write_all(&out).await {
                        let _ = reply_tx.send(Err(err.into()));
                        self.fail_all_pending(err);
                        return;
                    }

                    self.pending.push_back(reply_tx);
                }

                read = self.stream.read_buf(&mut self.read_buf), if !self.pending.is_empty() => {
                    let n = match read {
                        Ok(n) => n,
                        Err(err) => {
                            self.fail_all_pending(err);
                    ...
                    while let Some(reply) = parse_reply(&mut self.read_buf)? {
                        if let Some(reply_tx) = self.pending.pop_front() {
...
### ClientConnection internals

Use one `ClientConnection` that owns the backend socket:
...
impl ClientConnection {
        ...
        loop {
            tokio::select! {
                Some(cmd) = self.rx.recv() => {
                    if let Err(err) = self.write_command(cmd).await {
                        self.fail_all_pending(err);
                        return;
                    }
                }
    ...
    async fn write_command(&mut self, cmd: ClientCommand) -> Result<()> {
        match cmd {
...
1. **Change `Client` into a handle + `ClientConnection`.**
   Keep one backend TCP connection. `Client` is the cloneable producer handle;
   `ClientConnection` owns the stream, receiver, read buffer, and pending FIFO.
