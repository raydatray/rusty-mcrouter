...
- The **connection** (`rusty-mcrouter/src/proxy/connection.rs`) turns a
  `Parsed::MultiGet` into **one output `seq`**, fans out **N single-key gets**
  (one `route_one` per key) through the normal per-request path (so
...
flowchart TB
  ...
  SM -->|"route Get{k1}"| T1["sub task 1 (i=0)"]
  SM -->|"route Get{k2}"| T2["sub task 2 (i=1)"]
  SM -->|"route Get{k3}"| T3["sub task 3 (i=2)"]
...
The common single-key get allocates **no `Vec`**. In `parse_request` (`mod.rs`),
`get` returns its `Parsed` directly (early `return`) while every other command
funnels through a single `Parsed::One` wrap:

```rust
let single = match cmd {
    b"get" => return get::parse_request(buf, eol_idx),
    b"set" => set::parse_request(buf, eol_idx, line_end),
    // .. one arm per single-key command, each -> Result<Option<Request>> ..
    _ => { let _ = buf.split_to(total); Err(ProtocolError::Malformed("unknown command")) }
};
...
Routing one request is split so the spawned future borrows nothing from the
connection. `Connection::route_target` does the only `&self`-touching work and
hands it back as an **owned** `RouteTarget` (which proxy, same-thread?, a clone
of the local route); the free `async fn route_one` consumes that owned value, so
its future is `'static` and `spawn_local`-able:

```rust
struct RouteTarget { handle: ProxyHandle, same_thread: bool, route: Rc<dyn DynRoute> }
...
The split is load-bearing: an `async fn route_one(&self, …)` would capture `&self`
for the whole future, making it non-`'static` and un-`spawn_local`-able. Doing the
`&self` work synchronously in `route_target` and giving `route_one` only owned data
is what keeps the future spawnable while leaving its signature a plain `-> Reply`.
`submit_single` calls `route_one(self.route_target(&req), req)`, forwards
`(seq, reply)` to `completed_tx`, and uses no parent and no `Vec`.
...
fn submit_multiget(&self, seq: usize, keys: Vec<Bytes>) {
    ...
    for (i, key) in keys.into_iter().enumerate() {
        ...
        tokio::task::spawn_local(async move {
            let _ = sub_tx.send((i, route_one(target, req).await)).await;
        });
    }
    drop(sub_tx);
...
| each subreq routed independently | `route_one(route_target(&req), req)` per key |
...
6. **Tests don't use a `net::testing` counting backend.** The design referenced a
   ...
   for the real two-backend `Ch3` spanning test — no new crate dependency.
7. **Routing one request is `route_target` (`&self`) + a free `async fn route_one`.**
   The design sketched `route_one(&self, req) -> impl Future<Output = Reply>`.
   As-built, the `&self` work is isolated in `Connection::route_target`, which
   returns an owned `RouteTarget`; the free `async fn route_one(target, req) -> Reply`
   consumes it. That keeps the spawned future `'static` (an `async fn` over `&self`
   would borrow the connection and be un-`spawn_local`-able) and the signature a
   plain `-> Reply`.
...
- duplicate keys (`get dup dup`) → two `VALUE`s (no dedupe).
- single-key `get` takes the common (`submit_single`) path → one `VALUE`.
- a multiget and a following single get pipeline back in request order (a
  multiget occupies exactly one `seq` slot).

**Real backends** (`connection.rs`): `multiget_spans_two_real_backends_in_request_order`
builds a real `PoolRoute(Ch3)` over two `keyed_echo_backend`s and asserts a
ten-key multiget returns all hits in request-key order while the keys are split
across both backends (none lost or duplicated). The Docker integration test
`get_multi_key_returns_only_hits` (`tests/integration.rs`) now also exercises the
split through the full router.
...
| owned routing target | `RouteTarget` | `rusty-mcrouter/src/proxy/connection.rs` |
| per-request routing | `route_target` (`&self`) + free `route_one`, `submit_single` | `rusty-mcrouter/src/proxy/connection.rs` |
