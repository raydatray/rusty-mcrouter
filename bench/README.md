...
```text
CPU 0     load generator
CPU 1-2   rusty-mcrouter
...
- `pr.toml`: three paired repetitions including a 300k RPS latency path.
...
Invalid performance pairs are reported as `INCONCLUSIVE` but do not fail CI;
the smoke and fault suites still fail on their validity checks. Process,
protocol, and build failures always fail the workflow.
