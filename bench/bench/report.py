...
def comparison(records: list[dict]) -> tuple[str, bool]:
    ...
    rows = [
        "| scenario | valid pairs | rps ratio | 95% interval | p99 ratio | 95% interval | verdict |",
        "|---|---:|---:|---:|---:|---:|---|",
    ]
    ...
    for name, scenario_records in grouped.items():
        ...
        if name == "direct-control":
            ...
            if regression:
                ...
                verdict = "NEUTRAL"
        rows.append(
            f"| {name} | {valid_pairs} | {rps_mid:.3f} | {rps_low:.3f}..{rps_high:.3f} | "
            f"{p99_mid:.3f} | {p99_low:.3f}..{p99_high:.3f} | {verdict} |"
        )
