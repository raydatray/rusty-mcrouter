...
def test_comparison_surfaces_invalid_run_reasons() -> None:
    records = [
        _comparison_record("base", 0, 100, 10),
        _comparison_record("head", 0, 100, 10),
        _comparison_record("base", 1, 100, 10),
        _comparison_record("head", 1, 100, 10),
    ]
    records[-1]["validity"] = {
        "valid": False,
        "reasons": ["schedule lag p99 5795us > 5000us"],
    }
    markdown, failed = comparison(records)
    assert "| pool | 1 | 1 |" in markdown
    assert "head r1: schedule lag p99 5795us > 5000us" in markdown
    assert not failed
